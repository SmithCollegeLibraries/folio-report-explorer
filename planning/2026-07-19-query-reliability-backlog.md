...
- Full-backend release verification passes twice with identical counts: `BACKEND_PASS=1 PASSED_SCRIPTS=106` and `BACKEND_PASS=2 PASSED_SCRIPTS=106`. Each pass records the credential-dependent FOLIO PostgreSQL test as skipped rather than failed; existing PHP 8.5 reflection deprecations and reference-resolver fixture warnings remain non-failing.
